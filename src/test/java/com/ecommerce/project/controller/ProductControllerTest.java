...
class ProductControllerTest {
  ...
  void createProduct_returns201() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/admin/categories/1/products")
  ...
  void createProduct_categoryNotFound_returns404() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/admin/categories/99/products")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    objectMapper.writeValueAsString(
                        new ProductCreateRequest(
                            "Laptop Pro", "desc", 0, BigDecimal.ZERO, BigDecimal.ZERO))))
  ...
  void deleteProduct_returns200() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/admin/products/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(1))
        .andExpect(jsonPath("$.name").value("Laptop Pro"));
  }
  ...
  void getAllProducts_returns200() throws Exception {
    when(productService.getAllProducts(any(), any(), any(), any()))
  ...
  void getProductById_returns200() throws Exception {
    ...
    mockMvc
        .perform(get("/api/public/products/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(1))
        .andExpect(jsonPath("$.name").value("Laptop Pro"));
  }
  ...
  void getProductsByKeyword_returns200() throws Exception {
    ...
    mockMvc
        .perform(get("/api/public/products/keyword/laptop"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray())
        .andExpect(jsonPath("$.content.length()").value(1));
  }
  ...
  void getProductsByCategory_returns200() throws Exception {
    ...
    mockMvc
        .perform(get("/api/public/categories/1/products"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray())
        .andExpect(jsonPath("$.content.length()").value(1));
  }
